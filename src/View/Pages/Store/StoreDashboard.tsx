...
export class StoreDashboard extends React.Component<any, State> {
    ...
    async componentDidMount() {
        this.setState({tokenQuantities: [500, 1000, 1500, 2000, 2500, 5000, 7500, 20000]});
    }
        ...
        return <div>
            ...
            <div className={"card"}>
                <div className={"card-body"}>
                    <h5 className="card-title">Purchase Tokens With Crypto / RSGP</h5>
                    <p className={"card-text"}>
                        Wanting to purchase tokens with cryptocurrencies or RSGP?
                        Checkout the token shop run by some RSPeer staff members.
                    </p>
                    <p>If you would prefer an instant purchase, please use the paypal checkout below.</p>
                    <h6>
                        <a href={"https://discourse.rspeer.org/t/official-rspeer-tokenshop/1874/"} target={"_blank"}>View Token Shop</a>
            ...
            <div className="card">
                <div className="card-body">
                    <h5 className="card-title">Purchase Tokens With Paypal</h5>
                    ...
                    {this.state.tokenQuantities.map(q => {
                        return <button onClick={() => this.purchaseTokens(q)} type="button"
                                       ...
                                       className={"btn btn-info"}>{this.state.processing === q ? 'Processing...' : `${Util.formatNumber(q.toString())} Tokens`}</button>
                    })}
                </div>
            </div>
            <div className="card">
                <div className="card-body">
                    ...
                    <button onClick={this.purchaseUnlimitedInstances} type={"button"}
                            className={"btn btn-success"}>Purchase Unlimited Instances For {Util.formatNumber("7500")} Tokens
                    </button>
                    <br/><br/>
                </div>
            </div>
